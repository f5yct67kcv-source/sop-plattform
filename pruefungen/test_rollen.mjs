...
// "personal_schreiben" ist nicht "rechte_schreiben": Wer die Personalakte
// pflegen darf, kommt damit noch nicht an die Rollenvergabe. Dieser Fall
// stand bis ENT-684 neben der Personalnummern-Kachel und pruefte beide
// Haelften; die Kachel ist weg, die Aussage bleibt -- und sie ist die
...
try {
  ...
  await page.evaluate(() => go('betrieb'));
  await page.waitForTimeout(900);
  check('KRITISCH: "personal_schreiben" allein oeffnet die Rollenseite NICHT',
    !(await sichtbar('bkKachelRv')));
  meineRechte = ALLE_RECHTE; meineRollen = ['verwaltung'];
} catch (e) { check('Abschnitt personal_schreiben ist nicht rechte_schreiben: ' + e.message, false); }
