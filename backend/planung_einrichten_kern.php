...
// Rechenkern der Einsatzplanungs-Einrichtung (ENT-020, ENT-021), seit ENT-612
// getrennt von seinem urspruenglichen, einzigen Aufrufer.
//
// WARUM GETRENNT: Bis hierher lag die gesamte Logik direkt im Endpunkt
// api/planung_einrichten.php -- richtig, solange nur ein Mandant sie je
// gegen die EIGENE Verbindung (db()) aufrief. Seit der Betreiber-Bereich
// dieselbe Einrichtung zentral gegen JEDE Mandanten-Datenbank anstossen
// kann (api/betreiber_schema_pruefen.php, ENT-612), braucht es dieselbe
// Funktion an zwei Stellen -- "eine Definition, nicht zwei" (Hausregel).
// planung_einrichten_ausfuehren() kennt darum keine eigene Verbindung
// mehr: Sie bekommt ihr PDO von aussen.
//
// $mitBetreiberEbene steuert NUR den Abschnitt 3b unten (ENT-529, die
// Tabellen des Betreiber-Bereichs selbst). Der eigene Endpunkt eines
// Mandanten laesst ihn an -- seine Verbindung KANN zufaellig betreiber_db()
// sein (Rueckfall auf db(), solange getrennte Datenbanken fehlen, ENT-519).
// Der zentrale Betreiber-Lauf schaltet ihn je Mandant ab: Er richtet
// bewusst NUR die Betriebstabellen DIESES Mandanten ein, nie dessen (in
// Wahrheit gemeinsame) Betreiber-Ebene ein zweites Mal je Mandant --
// betreiber_schema_pruefen.php erledigt die Betreiber-Ebene selbst, einmal.
//
// NICHT NEU EINGERUECKT: Der Funktionsrumpf unten steht bewusst auf der
// urspruenglichen Einrueckstufe (Spaltenstufe 0), nicht auf Stufe 1 --
// mehrere Pruefungen (u. a. pruef_sql.php) lesen die eingebetteten
// CREATE-TABLE-Anweisungen ueber ein "\n)"-Muster, das eine zusaetzliche
// Einrueckung stillschweigend zerstoert haette.
...
if (!function_exists('hat_fremdschluessel')) {
