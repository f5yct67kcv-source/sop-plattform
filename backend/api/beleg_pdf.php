<?php
// Das unterschriebene PDF einer Offerte oder eines Vertrags der Mandantin am Link
// (ENT-688, Schritt 3).
//
...
try {
    ...
    $pdo = db();
    $s = $pdo->prepare('SELECT id, art, nummer FROM belege WHERE versand_token = ?');
    $s->execute([$token]);
    $b = $s->fetch(PDO::FETCH_ASSOC);
    if (!$b) { pdf_nicht_da(); }
    $pdf = beleg_pdf_gespeichert($pdo, '', (int)$b['id']);
