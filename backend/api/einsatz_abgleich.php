...
try {
    ...
    // Entfallene UND abgelehnte Zuteilungen zaehlen hier NICHT mit
    // (ENT-347, erweitert ENT-350/OP-348). Das ist keine GAV-Auslegung,
    // sondern eine Tatsache: Wer aus einer Schicht entfallen ist oder sie
    // abgelehnt hat, war dort nicht -- und hatte darum auch keinen Hin- und
    // Rueckweg dorthin. Die Frage aus GAV-AUS-010 ("welcher von mehreren
    // Einsaetzen ist DER eine Weg") stellt sich fuer einen Einsatz nicht,
    // den niemand angetreten hat. Vermerk dazu im Auslegungsregister bei
    // GAV-AUS-010.
    //
    // Ohne diese Zeile waere die Wirkung von ENT-347/350 zur Haelfte
    // verpufft: Die Schicht waere sichtbar unterbesetzt, ihr Auslagenersatz
    // aber weiterhin gesperrt -- wegen einer Zuteilung, die niemand
    // wahrgenommen hat.
    $tagKonflikt = $pdo->prepare(
        "SELECT COUNT(*) FROM einsatz_zuteilung z
         ...
         WHERE z.mitarbeiter_id = ? AND e.datum = ? AND e.status != 'abgesagt'
           AND z.zusage NOT IN ('entfallen', 'abgelehnt') AND e.id != ?"
    );
